...
\end{align}
so that in the particle-hole symmetric case the chemical potential to obtain a half-filled system is $-2K'(0^{+})+\sum_{j}U_{ij}/2$ (which is independent of $i$). The chemical potential is shifted internally by the solver, such that an input chemical of $\sum_{j}U_{ij}/2$ corresponds to half-filling. The latter is provided through the input parameters \verb#U# and \verb#MU# or input the corresponding input files.
